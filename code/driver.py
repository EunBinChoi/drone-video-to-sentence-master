...
def RNNGenCost(batch, model, params, misc):
  ...
  for i,pair in enumerate(batch):
    ...
    loss_cost += - np.sum(np.log(1e-20 + P[list(range(len(gtix))),gtix])) # note: add smoothing to not get infs
    logppl += - np.sum(np.log2(1e-20 + P[list(range(len(gtix))),gtix])) # also accumulate log2 perplexities
    logppln += len(gtix)
...
def main(params):
  ...
  print('model has keys: ' + ', '.join(list(model.keys())))
  ...
  max_iters = int(max_epochs * num_iters_one_epoch)
